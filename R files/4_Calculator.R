...
frame.w.sound = data.frame(frame_nums, gap)
avg.turn2=0
...
for (j in seq(start.50,T,1)) {
  ...
  if (gap.v2[j, 2] == 0 & gap.v2[j+42, 2] == 0) {
    end.50 = j
    print(end.50)
    break
  }
...
for (i in seq(start.55, T, 1)) {
  if (gap.v2[i, 2] == 0 & gap.v2[i+42, 2] == 0) {
    end.55 = i
    print(end.55)
    break
  }
...
baseline = mean(angle.frame[((end.65 + 10)/12):((start.70 - 10)/12)])
cat("Baseline turn angle is:", baseline, "\n")
cat("Start burst1 is:", start.70, "\n")
...
for (i in seq(start.70, T, 1)) {
  if (gap.v2[i, 2] == 0 & gap.v2[i+5, 2] == 0) {
    end.burst1 = i
    #print(end.burst1)
...
turn.angles = c(max.burst1, max.burst2, max.burst3, max.burst4, max.burst5)
avg.max.burst = mean(turn.angles)
cat("Average max turn angle is:", avg.max.burst, "\n\n")
# 
...
diff.turn = avg.max.burst - baseline
cat("Differential turn angle is:", diff.turn, "\n\n")
#print(file_name_csv)
#results = matrix(nrow = length(stripped_files), ncol = 3)


results[num.files, 1] = file_name_csv
results[num.files, 2] = diff.turn
...
write.table(frame.w.sound, file = "frames_w_sound.txt", sep = "\t", row.names = FALSE, col.names = FALSE)
